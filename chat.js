...
(function () {
  ...
  function botReply(q) {
    ...
    if (has("plan", "itinerary", "schedule", "overview", "summary", "route", "trip about")) return `🗺️ The plan: <b>Istanbul → Cappadocia</b> over ~8 nights in September — ~4 days of Istanbul's culture & food, then ~3 days of Cappadocia's balloons, valleys and cave hotels. Your picked activities auto-slot into a day-by-day itinerary, grouped by neighbourhood with walking times.`;
    ...
    if (has("eat", "food", "restaurant", "dinner", "lunch", "breakfast", "drink", "bar", "pub", "cafe", "coffee")) {
      ...
      return `🍽️ Top tables — <b>Istanbul</b>: ${ist}. <b>Cappadocia</b>: ${cap}. For bars/pubs and the full list (with Maps links + your votes), see the Eat & drink section.`;
    }
    if (has("hotel", "stay", "sleep", "accommodat", "where to stay")) return `🏨 Boutique 4★ in the $120–200/night range: ${HOTELS.map((h) => `${h.icon} ${h.city}`).join(" · ")}. The Stay section has booking links.`;
    if (has("when", " date", "dates", "what month", "september", "how long", "how many days")) return `🗓️ <b>${fmt(state.dates.start)} – ${fmt(state.dates.end)}</b>, about 8 nights in early–mid September — warm, sunny and balloon-friendly.`;
    ...
    if (has("coast", "beach", "antalya", "fethiye", "oludeniz", "ölüdeniz", "lagoon", "paraglid")) return `This trip is <b>Istanbul + Cappadocia</b> — we dropped the coast to keep an 8-night pace relaxed. If you'd like the beach back in, just say so and I can swap it in.`;
    if (has("activit", "things to do", "what to do", " do ", "see ", "experien")) return `Tell me a place — try “what can we do in Cappadocia?” Highlights overall: sunrise balloon 🎈, Bosphorus sunset cruise ⛴️, ATV through the valleys 🏍️, a Turkish hammam 🛁.`;
    ...
    if (has("weather", "hot", "cold", "rain", "pack", "wear")) return `September in Türkiye is warm and mostly dry — pleasant in Istanbul, with cooler mornings in Cappadocia (great for the balloon). Pack light layers and comfy walking shoes.`;
    if (has("thank", "thanks", "cheers")) return `Anytime! İyi yolculuklar — happy travels! 🇹🇷`;
    return `I'm the built-in helper, so I might not have that one. Try: <i>visas, flights, what to do in Istanbul or Cappadocia, where to eat, the budget, or what you've picked.</i>${(typeof CONFIG !== "undefined" && CONFIG.chatApi) ? "" : "<br><small>💡 Want real conversation? Connect an AI key — see SETUP.md §5.</small>"}`;
  }
  ...
  function buildContext() {
    ...
    return [
      `You are a warm, concise travel assistant embedded in a trip-planning website for an ~8-night September trip to Türkiye (11–19 Sept) for two friends: ${state.names.a} (flying from San Francisco, US passport, visa-free) and ${state.names.b} (flying from New Delhi, needs a Turkish visa).`,
      `The trip is Istanbul → Cappadocia (the coast was dropped to keep a relaxed pace), medium budget (~$${budgetTotal().toLocaleString()}/person).`,
      `Activities span those two regions (e.g. Hagia Sophia, Bosphorus cruise, a hammam, the Cappadocia sunrise hot-air balloon, valley ATV rides, cave-hotel sunsets). There are curated restaurant/bar lists per region.`,
